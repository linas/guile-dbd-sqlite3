...
(define (check headline result expected)
  ...
  (if (equal? result expected)
	...
	(newline))
      (begin
	(display " FAILED: ")
	(display (list "equal?" result expected))
	(newline))))
...
(check
 ...
 (begin
   (dbi-query db-obj
	      "insert into testtable ('id', 'name') values('33', 'testname1')")
   (dbi-query db-obj
	      "insert into testtable ('id', 'name') values('34', 'testname1')")
   (dbi-query db-obj
	      "insert into testtable ('id', 'name') values('44', 'testname2')")
   (dbi-get_status db-obj))
 '(0 . "query ok"))
...
(check
 "open/close not leaking file descriptors, not segfaulting"
 (do ((i 0 (1+ i))
      (result #f))
     ((> i 2048)
      result)
   (let ((db-obj (dbi-open "sqlite3" db-path)))
     (dbi-query db-obj "SELECT 42")
     ...
     (dbi-query db-obj "SELECT * FROM testtable")
     (dbi-query db-obj "SELECT * FROM testtable")
     (dbi-query db-obj "SELECT * FROM non-existent")
     (dbi-query db-obj "COMMIT TRANSACTION")
     (when (= i 1024)
       (set! result (dbi-get_status db-obj)))
     (dbi-close db-obj)))
 '(0 . "query ok"))
